...
{
    /// <inheritdoc />
    public partial class FixCurrentPeriodViewOffsets : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            RecreateCurrentPeriodViews(migrationBuilder, offset: 0);
        }

        /// <inheritdoc />
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            RecreateCurrentPeriodViews(migrationBuilder, offset: 1);
        }

        private static void RecreateCurrentPeriodViews(MigrationBuilder migrationBuilder, int offset)
        {
            migrationBuilder.Sql($"""
                ...
                LEFT JOIN experimental_specs_wl_weekly w
                    ON c.uuid = w.uuid
                    AND w.week_start_date = (SELECT id FROM time_week ORDER BY id DESC OFFSET {offset} LIMIT 1);
                """);
            ...
            migrationBuilder.Sql($"""
                ...
                LEFT JOIN experimental_specs_wl_daily d
                    ON c.uuid = d.uuid
                    AND d.day_start_date = (SELECT id FROM time_day ORDER BY id DESC OFFSET {offset} LIMIT 1);
                """);
            ...
            migrationBuilder.Sql($"""
                ...
                SELECT
                    ...
                    (SELECT id FROM time_day ORDER BY id DESC OFFSET {offset} LIMIT 1) AS day_start_date,
            ...
            migrationBuilder.Sql($"""
                ...
                SELECT
                    ...
                    c.weight AS current_weight,
                    d.weight AS previous_weight,
                    (SELECT id FROM time_week ORDER BY id DESC OFFSET {offset} LIMIT 1) AS week_start_date,
                    CASE WHEN d.uuid IS NOT NULL THEN c.weight - d.weight ELSE 0 END AS weight_change
                FROM base_weights c
                LEFT JOIN base_weights_weekly d
                    ON c.uuid = d.uuid
                    AND d.week_start_date = (SELECT id FROM time_week ORDER BY id DESC OFFSET {offset} LIMIT 1);
                """);
        }
    }
}
