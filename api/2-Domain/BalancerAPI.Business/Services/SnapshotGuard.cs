...
internal static class SnapshotGuard
{
    ...
    public static async Task EnsureSpecsWlSeasonalAsync(
        BalancerDbContext db,
        IReadOnlyCollection<Guid> uuids,
        CancellationToken cancellationToken)
    {
        var currentSeasonId = await db.TimeSeasons
            .OrderByDescending(x => x.Id)
            .Select(x => (int?)x.Id)
            .FirstOrDefaultAsync(cancellationToken);
        ...
        var alreadySnapshotted = await db.ExperimentalSpecsWlSeasonal
            .Where(x => x.SeasonStartDate == currentSeasonId && uuids.Contains(x.Uuid))
            .Select(x => x.Uuid)
            .ToHashSetAsync(cancellationToken);
        ...
        db.ExperimentalSpecsWlSeasonal.AddRange(
            rows.Select(r => ToSeasonalSnapshot(r, uncountRows.GetValueOrDefault(r.Uuid), currentSeasonId.Value)));
    }
    ...
    /// <inheritdoc cref="ToDailySnapshot"/>
    public static ExperimentalSpecsWlSeasonal ToSeasonalSnapshot(
        ExperimentalSpecsWlColumns c,
        ExperimentalSpecsWlColumns? u,
        int seasonStartDate) =>
        new()
